...
## 2026-07-04 - 强化电脑端布局和搜题体验

### 本次修改内容

- 将电脑端应用外壳从 1320px 预览框扩展到最高 1680px，更接近完整桌面管理台。
- 搜题页重构为三栏工作区：左侧科目/模式控制，中间题库文件夹，右侧图片选择、搜索和结果。
- 修复搜题入口太靠后的问题：选完科目后，可以直接上传图片或输入关键词搜题。
- 修复多余按钮干扰的问题：删掉更新、下载清单、准备题库等不需要用户操作的按钮。
- 优化试卷列表显示：QP 和 MS 分开成两个文件，标题只看卷号和文件类型，小字再看完整代码和类别。
- 修复试卷链接跳转方式，打开题目和答案更直接。
- 数学题库浏览器在电脑端改为文件管理器布局：年份和月份在左，卷号、试卷和答案入口在右侧网格中滚动。
- 首页电脑端强化横向管理布局，今日安排作为中间主工作区，考试/搜题/番茄钟分列管理。
- 日历电脑端改为三列：左侧今日概览和月份控制，中间等比例时间轴，右侧当天详情。
...
- 这版进一步依赖桌面端 CSS 管理多列滚动区域，个别中等宽度屏幕可能还需要微调列宽。
- 搜题页 JSX 结构变化较大，但业务逻辑和搜索流程未改；如果后续重做搜题页，需要同步维护三栏布局 class。
- 当前环境没有可用的内置浏览器实例，无法自动截图验收；已用 TypeScript 和生产构建验证。

### 建议 commit message

```text
feat: make desktop layout a horizontal management workspace
```
