...
export function TodayPage({
  ...
  const todayTasks = todayTaskPool.slice(0, 4);
  ...
  const todaySchedule = timelineItems.slice(0, 12);
  ...
  return (
    <main className="today-page px-5 pb-32 pt-7 md:grid md:grid-cols-1 md:items-start md:gap-5 md:px-6 md:pb-8 md:pt-6 lg:min-h-full lg:grid-cols-[minmax(0,1fr)_minmax(360px,0.82fr)] lg:grid-rows-[auto_auto] lg:content-start lg:gap-5 lg:overflow-visible lg:px-7 xl:grid-cols-[minmax(0,0.98fr)_minmax(420px,0.82fr)]">
      ...
      <section className="mb-4 grid grid-cols-2 gap-2 md:mb-0 md:gap-3 lg:hidden">
        <QuickStat
      ...
      {examSubject ? (
        <section className="rounded-[30px] border border-[#F7CACA] bg-white p-4 shadow-soft md:min-h-[260px] lg:hidden">
      ...
      <section className="mt-5 rounded-[28px] border border-white/80 bg-white p-4 shadow-soft md:mt-0 lg:order-1 lg:flex lg:min-h-[420px] lg:flex-col lg:overflow-hidden">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <p className="text-sm font-black text-ink">今日日历</p>
            <p className="text-xs text-muted">课表 + 定时安排 · 点击进入完整日历</p>
          </div>
          <button
            type="button"
        ...
        <div className="space-y-3 lg:min-h-0 lg:flex-1 lg:overflow-y-auto lg:pr-1">
          ...
          {todaySchedule.length === 0 ? (
            <div className="rounded-[22px] bg-cream p-5 text-center text-sm font-bold text-muted">今天暂时没有定时日程</div>
          ) : null}
        </div>
      </section>

      <section className="mt-5 md:mt-0 lg:order-3">
...
function HomePomodoro({
  ...
  return (
    <div className="flex h-full min-h-0 flex-col lg:flex-1">
      ...
      <div className="mb-3 grid grid-cols-2 gap-2 lg:mb-4">
        <div className="rounded-[22px] bg-[#F1F4F8] p-3">
          <div className="mb-2 flex items-center gap-1.5 text-muted">
            <Clock3 size={14} />
            <span className="text-[11px] font-black">今日专注</span>
          </div>
          <p className="text-xl font-black text-ink lg:text-2xl">{minutesToText(focusMinutes)}</p>
        </div>
        <div className="rounded-[22px] bg-[#E8F0E6] p-3">
          <div className="mb-2 flex items-center gap-1.5 text-[#166534]">
            <Trophy size={14} />
            <span className="text-[11px] font-black">完成组数</span>
          </div>
          <p className="text-xl font-black text-ink lg:text-2xl">{sessionCount} 组</p>
        </div>
      </div>

      <div className="grid grid-cols-[104px_minmax(0,1fr)] gap-3 rounded-[24px] bg-[#F1F4F8] p-3 lg:min-h-0 lg:flex-1 lg:grid-cols-1 lg:grid-rows-[minmax(0,1fr)_auto] lg:p-4">
        <div className="grid h-28 place-items-center rounded-[22px] bg-white text-center shadow-soft lg:h-auto lg:min-h-[260px]">
          <div>
            <p className="text-[11px] font-black text-muted">{running ? "专注中" : `${settings.focusDuration} 分钟`}</p>
            <p className="mt-1 text-3xl font-black tracking-normal text-ink lg:text-6xl">{formatSeconds(secondsLeft)}</p>
          </div>
        </div>
        <div className="min-w-0 lg:pt-1">
          <input
            ...
            placeholder="自定义学习任务"
            className="h-10 w-full rounded-[16px] border border-white bg-white px-3 text-sm font-black text-ink outline-none focus:border-ink/20 lg:h-12"
          />
          <div className="mt-3 grid grid-cols-3 gap-2">
            <button
              type="button"
              onClick={() => setRunning(true)}
              className="grid h-10 place-items-center rounded-full bg-sky text-ink shadow-pill lg:h-12"
              aria-label="开始番茄钟"
            >
              <Play size={16} />
            </button>
            <button
              ...
              className="grid h-10 place-items-center rounded-full bg-white text-ink shadow-soft lg:h-12"
              aria-label="暂停番茄钟"
            >
              <Pause size={16} />
            </button>
          ...
          <button
            ...
            className="mt-2 flex h-10 w-full items-center justify-center gap-1 rounded-full bg-mint text-xs font-black text-ink shadow-soft lg:h-12"
          >
            <Trophy size={15} />
            完成本轮
          </button>
