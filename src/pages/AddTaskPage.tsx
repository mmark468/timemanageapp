...
function titlePlaceholder(type: TaskType) {
  ...
  if (type === "个人安排") return "例如：社团活动 / 通勤 / 运动";
...
export function AddTaskPage({ subjects, defaultType = "个人安排", onBack, onSave }: AddTaskPageProps) {
  ...
  return (
    <main className="px-5 pb-36 pt-7">
      ...
      <section className="space-y-4 rounded-[30px] bg-white p-5 shadow-soft">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Field label={isHoliday ? "开始日期" : "日期"} type="date" value={date} onChange={setDate} />
          {isHoliday ? (
      ...
      <div className="fixed bottom-24 left-1/2 z-20 w-full max-w-[390px] -translate-x-1/2 px-5">
        <button
          type="button"
          onClick={saveTask}
