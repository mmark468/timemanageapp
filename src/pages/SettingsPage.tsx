...
export function SettingsPage({
  ...
  return (
    <main className="settings-page px-5 pb-28 pt-7 md:grid md:grid-cols-2 md:items-start md:gap-5 md:px-6 md:pb-8 md:pt-6 lg:px-7 xl:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
      <PageHeader title="设置" subtitle="个人信息和当前学习范围。" onBack={onBack} />

      <section className="rounded-[28px] bg-white p-5 shadow-soft md:grid md:grid-cols-2 md:gap-4">
        ...
        <label className="mt-4 block md:mt-0">
      ...
      <section className="visual-style-panel mt-4 rounded-[26px] bg-white p-4 shadow-soft md:mt-0">
      ...
      <button
        type="button"
        onClick={save}
        className="mt-5 flex h-12 w-full items-center justify-center gap-2 rounded-full bg-ink text-sm font-black text-white shadow-pill md:col-span-2 md:mt-0 md:max-w-sm md:justify-self-end"
      >
        <Save size={17} />
        保存设置
      </button>
